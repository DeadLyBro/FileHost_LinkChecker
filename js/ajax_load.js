	...
	$("#link1").on("change", function(){
		...
		if (url == "") {
			...
			if (!preg_match.test(url)) {
				$("#status1").html("<span class=\"badge badge-danger\">Not vaild URL format</span>").slideDown("fast");
	...
	$.ajax({
		...
		success: function(string) {	
			...
			if (string.status == "good_link") {
				...
				$("#sttpass"+id).html("<span class=\"badge badge-danger\">Requre Password</span>").slideDown("fast");
				$('#submit').attr('disabled', 'disabled');
			}
			else if (string.status == "wrong_password") {
				document.getElementById("pass"+id).style.border="1px solid red";
				$("#sttpass"+id).html("<span class=\"badge badge-danger\">Wrong Password. Try again</span>").slideDown("fast");
				$('#submit').attr('disabled', 'disabled');
			}
